...
LOG_MODULE_REGISTER(udynlink, CONFIG_UDYNLINK_LOG_LEVEL);
...
static uint8_t *get_code_pointer(const udynlink_module_t *p_mod)
{
	...
	if (UDYNLINK_LOAD_GET_MODE(p_mod) == UDYNLINK_LOAD_MODE_COPY_CODE)
	{																		   // the code is after the LOT in RAM.
		return (uint8_t *)p_mod->p_ram + p_header->num_lot * sizeof(uint32_t); // the code is after the LOT in RAM.
	}
	else
	{ // the code is after the module header, the relocations and the symbol table
...
static uint8_t *get_data_pointer(const udynlink_module_t *p_mod)
{
	...
	default:
		LOG_ERR("Invalid load mode %d", (int)UDYNLINK_LOAD_GET_MODE(p_mod));
		return NULL;
	}
}
...
static udynlink_sym_t *get_sym_at(const udynlink_module_header_t *p_header, uint32_t index, udynlink_sym_t *p_sym)
{
	...
	if (index >= *p_symt)
	{ // first word in the symbol table is the number of entries
		return NULL;
	}
	...
	{
		LOG_ERR("Module name symbol doesn't have the correct type!");
		return NULL;
	}
	// Get name pointer (if available)
	if (p_sym->type != UDYNLINK_SYM_TYPE_LOCAL)
	{ // local symbols don't have names
...
static udynlink_sym_t *offset_sym(const udynlink_module_t *p_mod, udynlink_sym_t *p_sym)
{
	...
	LOG_INF("Symbol %-20s relocated relative to %s, orig value is 0x%08X, new value is 0x%08X", p_sym->name, p_sym->location == UDYNLINK_SYM_LOCATION_CODE ? "code" : "data", prev_val, p_sym->val);
	return p_sym;
}
...
udynlink_error_t udynlink_load_module(udynlink_module_t *p_mod, const void *base_addr, void *load_addr, uint32_t load_size, udynlink_load_mode_t load_mode)
{
	...
	LOG_INF("Processing module at %p named '%s' with load mode %d", base_addr, udynlink_get_module_name(p_mod), (int)load_mode);

	// Allocate RAM or check given RAM region, as needed
	uint32_t ram_size = udynlink_get_ram_size(p_mod);
	if (ram_size > 0)
	{ // is any RAM needed at all?
		if (load_addr == NULL)
		{ // RAM must be allocated
			...
			LOG_INF("Allocated %u bytes for module at %p", ram_size, base_addr);
		}
		else
		{ // check if the user-provided RAM region is large enough
		...
		LOG_INF("RAM area for module at %p is at %p (%u bytes)", base_addr, ram_addr, ram_size);
	}
	else
	{
		p_mod->p_ram = NULL;
		LOG_INF("Awesome! Module %p doesn't need any RAM", base_addr);
	}
	...
	if (load_mode == UDYNLINK_LOAD_MODE_COPY_ALL)
	{
		...
		LOG_INF("Copied module at %p to RAM at %p (%u bytes)", base_addr, p_temp8, load_size + p_header->code_size + p_header->data_size);
	...
	else if (load_mode == UDYNLINK_LOAD_MODE_COPY_CODE)
	{
		...
		LOG_INF("Copied code and data of module %p to RAM at %p (%u bytes)", base_addr, p_temp8, p_header->code_size + p_header->data_size);
	}
	else
	{
		// XIP mode: copy only data
		memcpy(p_temp8, (const uint8_t *)base_addr + load_size + p_header->code_size, p_header->data_size);
		LOG_INF("Copied data of module %p to RAM at %p (%u bytes)", base_addr, p_temp8, p_header->data_size);
	}
	...
	LOG_INF("LOT base: %p, .data starts at %p, .code starts at %p", p_lot, p_data, get_code_pointer(p_mod));
	...
	for (uint32_t i = 0; i < p_header->num_rels; i++)
	{
		...
		if (get_sym_at(p_mod->p_header, symt_offset, &sym) == NULL)
		{ // symbol table offset is out of range, shouldn't happen
		...
		case UDYNLINK_SYM_TYPE_EXPORTED:
			// LOG_INF("Applying relocation for symbol at index %u, name=%s, type=%d, data_reloc=%d at lot_offset=%u, value=%08X", symt_offset, sym.name, sym.type, sym.location, lot_offset, sym.val);
		...
		case UDYNLINK_SYM_TYPE_EXTERN:
			...
			LOG_INF("Symbol %-20s relocated extern   at index %02u,  lot_offset=%02u value=0x%08x", sym.name, symt_offset, lot_offset, sym_addr);
			if (sym_addr > 0)
			{
			...
			{
				LOG_ERR("Unable to resolve relocation for extern symbol '%s'", sym.name);
...
exit:
	if (res != UDYNLINK_OK)
	{ // there's an error, so cleanup allocated structures and memory
		LOG_ERR("internal error: %s", error_codes[(int)res]);
		if ((p_mod->p_ram != NULL) && !UDYNLINK_LOAD_IS_FOREIGN_RAM(p_mod))
		{ // free allocated memory
			udynlink_external_free(p_mod->p_ram);
			LOG_ERR("Deallocated memory area at %p", p_mod->p_ram);
		}
		if (p_mod != NULL)
		{ // mark entry in module table as "free"
...
udynlink_error_t udynlink_unload_module(udynlink_module_t *p_mod)
{
	...
	{
		LOG_ERR("internal error: %s", error_codes[(int)UDYNLINK_ERR_INVALID_MODULE]);
		return UDYNLINK_ERR_INVALID_MODULE;
	}
	LOG_INF("Unloading module at %p", p_mod);
	if ((p_mod->p_ram != NULL) && !UDYNLINK_LOAD_IS_FOREIGN_RAM(p_mod))
	{ // free allocated memory
		udynlink_external_free(p_mod->p_ram);
		LOG_INF("Deallocated memory area at %p", p_mod->p_ram);
	}
	mark_module_free(p_mod);
	return UDYNLINK_OK;
}
...
udynlink_sym_t *udynlink_lookup_symbol(const udynlink_module_t *p_mod, const char *name, udynlink_sym_t *p_sym)
{
	...
	if (p_mod != NULL)
	{ // but consider only the given one if not NULL
		idx = 0;
		while (get_sym_at(p_mod->p_header, idx++, p_sym) != NULL)
		{ // iterate through module's symbol table
			if (!strcmp(p_sym->name, name))
			{									 // symbol found
